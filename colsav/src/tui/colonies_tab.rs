...
pub fn render(frame: &mut Frame, area: Rect, save: &SaveFile, state: &mut TableState) {
    ...
    .header(
        Row::new(vec!["#", "Name", "Nation", "Pop", "Location"]).style(theme::header()),
    )
    .block(
        Block::bordered()
            .title(" Colonies ")
    ...
    let detail = save.colonies.get(selected).map(colony_detail).unwrap_or_default();
...
fn colony_detail(colony: &Colony) -> Text<'static> {
    let mut lines = vec![
        Line::from(vec![Span::styled(colony.name().to_string(), theme::header())]),
        Line::from(format!("Nation: {}", enum_name::<NationType>(colony.nation_id))),
        ...
        Line::from(format!("Constructing: {}", constructable_name(colony.building_in_production))),
        Line::from(""),
        Line::from(vec![Span::styled("Storage", theme::header())]),
    ];

    for (idx, goods_name) in GOODS_NAMES.iter().enumerate() {
        lines.push(Line::from(format!("{goods_name:<11}: {:>6}", colony.stock[idx])));
    }
...
fn building_names(buildings: &colonization_sav::raw::colony::Buildings) -> Vec<String> {
    ...
    if buildings.fortification > 0 {
        names.push(match buildings.fortification {
            1 => "Stockade",
            2 => "Fort",
            3 => "Fortress",
            _ => "Fortification",
    ...
    if buildings.armory > 0 {
        names.push(match buildings.armory {
            1 => "Armory",
            2 => "Magazine",
            3 => "Arsenal",
            _ => "Armory",
    ...
    if buildings.docks > 0 {
        names.push(match buildings.docks {
            1 => "Docks",
            2 => "Drydock",
            3 => "Shipyard",
            _ => "Docks",
    ...
    if buildings.town_hall > 0 {
        names.push(match buildings.town_hall {
            1 => "Town Hall",
            2 => "Town Hall",
            3 => "Town Hall",
            _ => "Town Hall",
    ...
    if buildings.schoolhouse > 0 {
        names.push(match buildings.schoolhouse {
            1 => "Schoolhouse",
            2 => "College",
            3 => "University",
            _ => "Schoolhouse",
    ...
    if buildings.printing_press > 0 {
        names.push(match buildings.printing_press {
            1 => "Printing Press",
            2 => "Newspaper",
            _ => "Printing Press",
        }
    ...
    if buildings.weavers_house > 0 {
        names.push(match buildings.weavers_house {
            1 => "Weaver's House",
            2 => "Weaver's Shop",
            3 => "Textile Mill",
            _ => "Weaver's House",
    ...
    if buildings.tobacconists_house > 0 {
        names.push(match buildings.tobacconists_house {
    ...
    if buildings.rum_distillers_house > 0 {
        names.push(match buildings.rum_distillers_house {
    ...
    if buildings.fur_traders_house > 0 {
        names.push(match buildings.fur_traders_house {
    ...
    if buildings.carpenters_shop > 0 {
        names.push(match buildings.carpenters_shop {
            1 => "Carpenter's Shop",
            2 => "Lumber Mill",
            _ => "Carpenter's Shop",
        }
    ...
    if buildings.church > 0 {
        names.push(match buildings.church {
            1 => "Church",
            2 => "Cathedral",
            _ => "Church",
        }
    ...
    if buildings.blacksmiths_house > 0 {
        names.push(match buildings.blacksmiths_house {
