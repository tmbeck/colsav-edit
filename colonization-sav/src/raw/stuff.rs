...
impl Stuff {
    ...
    pub fn read(data: &[u8]) -> Result<Self> {
        ...
        fa.populations.copy_from_slice(&data[pos..pos + 4]); pos += 4;
        fa.unknown36ab.copy_from_slice(&data[pos..pos + 4]); pos += 4;
        fa.merchant_marine.copy_from_slice(&data[pos..pos + 4]); pos += 4;
        fa.ship_counts.copy_from_slice(&data[pos..pos + 4]); pos += 4;
        ...
        let show_colony_prod_quantities = data[pos]; pos += 1;

        let unknown_tribe_data_1 = TribeDataBlock::read(&data[pos..]); pos += TribeDataBlock::SIZE;
        let unknown_tribe_data_2 = TribeDataBlock::read(&data[pos..]); pos += TribeDataBlock::SIZE;
        let tribe_dwelling_count = TribeDataBlock::read(&data[pos..]); pos += TribeDataBlock::SIZE;
        let unknown_tribe_data_4 = TribeDataBlock::read(&data[pos..]); pos += TribeDataBlock::SIZE;
        let unknown_tribe_data_5 = TribeDataBlock::read(&data[pos..]); pos += TribeDataBlock::SIZE;
        let unknown_tribe_data_6 = TribeDataBlock::read(&data[pos..]); pos += TribeDataBlock::SIZE;

        let mut unknown36b = [0u8; 104];
        unknown36b.copy_from_slice(&data[pos..pos + 104]);
        pos += 104;

        let selector_x = u16::from_le_bytes([data[pos], data[pos + 1]]); pos += 2;
        let selector_y = u16::from_le_bytes([data[pos], data[pos + 1]]); pos += 2;
        let zoom_level = data[pos]; pos += 1;
        let unknown37 = data[pos]; pos += 1;
        let viewport_x = u16::from_le_bytes([data[pos], data[pos + 1]]); pos += 2;
        let viewport_y = u16::from_le_bytes([data[pos], data[pos + 1]]);

        Ok(Stuff {
            unknown34, all_unit_counts, unknwn_nation_data_35,
            unknown36aa, foreign_affairs: fa,
            unknwn_nation_data_36ac1, unknwn_nation_data_36ac2,
            unknwn_nation_data_36ac3, unit_counts,
            unknown36ac, average_colony, show_colony_prod_quantities,
            unknown_tribe_data_1, unknown_tribe_data_2,
            tribe_dwelling_count, unknown_tribe_data_4,
            unknown_tribe_data_5, unknown_tribe_data_6,
            unknown36b, selector_x, selector_y, zoom_level,
            unknown37, viewport_x, viewport_y,
        })
    }
    ...
    pub fn write(&self) -> Vec<u8> {
        ...
        // Foreign affairs
        buf[pos..pos + 4].copy_from_slice(&self.foreign_affairs.populations); pos += 4;
        buf[pos..pos + 4].copy_from_slice(&self.foreign_affairs.unknown36ab); pos += 4;
        buf[pos..pos + 4].copy_from_slice(&self.foreign_affairs.merchant_marine); pos += 4;
        buf[pos..pos + 4].copy_from_slice(&self.foreign_affairs.ship_counts); pos += 4;
        ...
        buf[pos] = self.show_colony_prod_quantities; pos += 1;

        self.unknown_tribe_data_1.write(&mut buf[pos..]); pos += TribeDataBlock::SIZE;
        self.unknown_tribe_data_2.write(&mut buf[pos..]); pos += TribeDataBlock::SIZE;
        self.tribe_dwelling_count.write(&mut buf[pos..]); pos += TribeDataBlock::SIZE;
        self.unknown_tribe_data_4.write(&mut buf[pos..]); pos += TribeDataBlock::SIZE;
        self.unknown_tribe_data_5.write(&mut buf[pos..]); pos += TribeDataBlock::SIZE;
        self.unknown_tribe_data_6.write(&mut buf[pos..]); pos += TribeDataBlock::SIZE;

        buf[pos..pos + 104].copy_from_slice(&self.unknown36b);
        pos += 104;

        buf[pos..pos + 2].copy_from_slice(&self.selector_x.to_le_bytes()); pos += 2;
        buf[pos..pos + 2].copy_from_slice(&self.selector_y.to_le_bytes()); pos += 2;
        buf[pos] = self.zoom_level; pos += 1;
        buf[pos] = self.unknown37; pos += 1;
        buf[pos..pos + 2].copy_from_slice(&self.viewport_x.to_le_bytes()); pos += 2;
