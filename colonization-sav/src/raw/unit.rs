...
impl Unit {
    pub fn read(data: &[u8]) -> Result<Self> {
        ...
        let unit_type = data[pos]; pos += 1;

        // nation_info bit_struct (1 byte)
        let ni_byte = data[pos]; pos += 1;
        ...
        let moves = data[pos]; pos += 1;
        let origin_settlement = data[pos]; pos += 1;
        let ai_plan_mode = data[pos]; pos += 1;
        let orders = data[pos]; pos += 1;
        let goto_x = data[pos]; pos += 1;
        let goto_y = data[pos]; pos += 1;
        let unknown18 = data[pos]; pos += 1;
        let holds_occupied = data[pos]; pos += 1;
        ...
        let turns_worked = data[pos]; pos += 1;
        let profession_or_treasure = data[pos]; pos += 1;
        ...
        Ok(Unit {
            x, y, unit_type, nation_id,
            vis_to_english, vis_to_french, vis_to_spanish, vis_to_dutch,
            unknown15_upper, damaged, moves, origin_settlement,
            ai_plan_mode, orders, goto_x, goto_y, unknown18,
            holds_occupied, cargo_items, cargo_hold,
            turns_worked, profession_or_treasure,
            next_unit_idx, prev_unit_idx,
        })
    }
    ...
    pub fn write(&self) -> Vec<u8> {
        ...
        buf[pos] = self.unit_type; pos += 1;

        // nation_info
        {
            let mut w = BitWriter::new(&mut buf[pos..pos + 1]);
        ...
        buf[pos] = self.moves; pos += 1;
        buf[pos] = self.origin_settlement; pos += 1;
        buf[pos] = self.ai_plan_mode; pos += 1;
        buf[pos] = self.orders; pos += 1;
        buf[pos] = self.goto_x; pos += 1;
        buf[pos] = self.goto_y; pos += 1;
        buf[pos] = self.unknown18; pos += 1;
        buf[pos] = self.holds_occupied; pos += 1;
        ...
        buf[pos] = self.turns_worked; pos += 1;
        buf[pos] = self.profession_or_treasure; pos += 1;
